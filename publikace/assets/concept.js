...
var editorEl = document.querySelector("#concept-editor");
var editToggle = document.querySelector("#edit-toggle");
var addButton = document.querySelector("#add-publication");
var form = document.querySelector("#publication-form");
var editIndexInput = document.querySelector("#edit-index");
var monthInput = document.querySelector("#publication-month");
var nameInput = document.querySelector("#publication-name");
var fileInput = document.querySelector("#publication-file");
var fileDrop = document.querySelector("#file-drop");
var cancelButton = document.querySelector("#cancel-edit");
var exportButton = document.querySelector("#export-json");
var resetButton = document.querySelector("#reset-local");
var passwordDialog = document.querySelector("#password-dialog");
var passwordForm = document.querySelector("#password-form");
var passwordInput = document.querySelector("#editor-password");
var passwordError = document.querySelector("#password-error");
var passwordCancel = document.querySelector("#password-cancel");
var editorStatus = document.querySelector("#editor-status");
var editorPasswordHash = "f9eb3caedd5e975fe700b461e63a6cf3d75aa6657d9a6d049252077a2801e0fa";
var basePublications = [];
var publications = [];
var shouldOpenEditor = new URLSearchParams(window.location.search).get("edit") === "1";
var isEditing = false;
var isEditorUnlocked = false;
var pendingUnlock = false;
var editorPassword = "";
var clonePublications = (items) => JSON.parse(JSON.stringify(items));
var hashValue = async (value) => {
...
var normalizePreviewName = (name) => {
  const cleanName = name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\.pdf$/i, "").replace(/_RGB$/i, "").replace(/[-_]?preview[-_]?s?$/i, "").replace(/[^a-zA-Z0-9._-]+/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "");
  return `${cleanName || "concept-publikace"}_preview.pdf`;
};
var formatDate = (date) => {
  const [year2, month] = String(date || "").split("-");
  return year2 && month ? `${month}/${year2}` : date || "";
};
...
var setEditorStatus = (message2, tone = "") => {
  if (!editorStatus) return;
  editorStatus.textContent = message2;
  editorStatus.dataset.tone = tone;
};
var publicManifest = (items = publications) => sortedPublications(items).map(({ localUrl, ...publication }) => publication);
var saveManifest = async (nextPublications = publications) => {
  setEditorStatus("Ukl\xE1d\xE1m seznam publikac\xED...", "busy");
  ...
  if (!response.ok) {
    throw new Error(payload.error || "Seznam publikac\xED se nepoda\u0159ilo ulo\u017Eit.");
  }
  basePublications = clonePublications(payload.publications);
  publications = clonePublications(payload.publications);
  setEditorStatus("Hotovo. Zm\u011Bny jsou ulo\u017Een\xE9 na webu.", "success");
  return publications;
};
var openHref = (publication) => {
  ...
  if (publication.url) {
    ...
    return `/publikace/viewer/?${params2.toString()}`;
  }
...
var createPublicationRow = (publication, index) => {
...
var renderPublications = () => {
  ...
  sorted.forEach((publication) => {
    const year2 = String(publication.date || "").slice(0, 4) || "Bez roku";
    if (!groups.has(year2)) groups.set(year2, []);
    groups.get(year2).push(publication);
  });
  groups.forEach((items, year2) => {
    const section = document.createElement("section");
    section.className = "concept-year-group";
    const heading = document.createElement("h2");
    heading.textContent = year2;
...
var setEditing = async (nextState) => {
  ...
  editToggle.textContent = isEditing ? "Zav\u0159\xEDt editaci" : "Upravit";
  renderPublications();
};
var resetForm = () => {
  form.reset();
...
var removePublication = (index) => {
  if (!window.confirm("Opravdu smazat tuto publikaci ze seznamu? PDF v \xFAlo\u017Ei\u0161ti z\u016Fstane zachovan\xE9.")) return;
  publications.splice(index, 1);
  saveManifest(publications).then(() => {
    resetForm();
    renderPublications();
...
resetButton.addEventListener("click", () => {
  publications = clonePublications(basePublications);
  resetForm();
  setEditorStatus("Na\u010Dten\xE1 posledn\xED ulo\u017Een\xE1 verze seznamu.", "");
  renderPublications();
});
...
form.addEventListener("submit", async (event) => {
  ...
  if (!existing && !file) {
    setEditorStatus("P\u0159i p\u0159id\xE1n\xED nov\xE9 publikace vyber PDF soubor.", "error");
    return;
  }
...
try {
  ...
  message2.className = "status-text error";
  message2.textContent = error.message;
  listEl.append(message2);
}
